...
final case class SocialNetwork(people: Vector[Person]) {
  def getFriendsOf(id: Int): Set[Int] = {
    // n = num of people
    // m = max num of followers
    // Retrieve all the people that $id follows:
    val follows = people(id).follows // n (on Seq), 1 (on Vector)

    //     Return only the people that follow $id back ("friends"):
    follows.filter { candidateId => // m
      val candidate = people(candidateId) // m (on Seq), 1 (on Vector)

      candidate.follows.contains(id) // m (on List), 1 (on Set)
    } // m^2 (on List), m (on Set)
  }
